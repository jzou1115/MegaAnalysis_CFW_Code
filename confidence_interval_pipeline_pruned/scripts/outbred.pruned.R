...
load.pruned.allele.dosages.for.chr2 <- function( chr, dir="/Net/dense/data/outbredmice/popgen/run5.5/prune_missing/pruned", suffix=".prunedgen.final.maf001.0.98.RData", chicago=FALSE ) {
  ...
  dups = duplicated(dosages$map$bp)
  dos = dosages$dosages[!dups,]
  map = data.frame(bp = dosages$map$bp[!dups], chr = dosages$map$chr[!dups], rsid = dosages$map$rsid[!dups])
  rownames(dos) = map$bp
  colnames(dos) = dosages$ids
  
  data=list( chr=dosages$chr, file=dosages$file, ids=dosages$ids, dosages=dos, map=map, type="allele" )
